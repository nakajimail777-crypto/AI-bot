begin;
alter table public.learning_items add column if not exists bookshelf_document_id uuid references public.knowledge_documents(id),
...
declare candidate public.learning_items; document_id uuid;
begin
  ...
  if candidate.bookshelf_document_id is not null then
    return jsonb_build_object('id',candidate.bookshelf_document_id,'alreadyAdopted',true);
  end if;
