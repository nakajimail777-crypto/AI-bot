...
import {prepareMarkdown,createBookshelfHandler} from '../api/bookshelf.js';
const env={RAG_IMPORT_TOKEN:'test-secret',SUPABASE_URL:'https://example.invalid',SUPABASE_SECRET_KEY:'test',GEMINI_API_KEY:'test'};
function response(){return {code:200,setHeader(){},status(c){this.code=c;return this;},json(d){this.data=d;return this;}};}
test('markdown validation, headings and long lines',()=>{assert.throws(()=>prepareMarkdown('../bad.md','x'));assert.throws(()=>prepareMarkdown('x.md',''));assert.throws(()=>prepareMarkdown('x.md','a'.repeat(40001)));const p=prepareMarkdown('number_1.md','# 数字1\n自分で決める\n## 龍導\n主体的に動く');assert.equal(p.title,'数字1');assert.equal(p.chunks.length,2);assert.equal(prepareMarkdown('long.md','a'.repeat(8000)).chunks.length,4);});
test('wrong secret makes no external requests',async()=>{const res=response();await createBookshelfHandler({env,fetcher:()=>{throw Error('unexpected');}})({method:'POST',body:{token:'bad',action:'list'}},res);assert.equal(res.code,401);});
