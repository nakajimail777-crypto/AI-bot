...
async function run({admin=true,candidate={id,learning_text:'他の対話でも使える学び',updated_at:stamp},body={candidateId:id,expected_updated_at:stamp},embeddingError=false,rpcError=false}={}){
...
test('retry of adopted item avoids duplicate embeddings and registration',async()=>{const r=await run({candidate:{id,updated_at:stamp,bookshelf_document_id:id}});assert.equal(r.res.data.alreadyAdopted,true);assert.equal(r.embedded.length,0);assert.equal(r.calls.length,2);});
