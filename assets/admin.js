(() => {
  ...
  const state = {page:'dashboard',offset:0,nextOffset:null,version:0,detailVersion:0,authorized:false,userId:null};
  ...
  async function renderPage() {
    ...
    try {
      ...
      if (page === 'dashboard') {
        ...
        const result = await api('conversations',{offset:state.offset});
        if (version !== state.version) return;
        state.nextOffset = result.nextOffset;
        html = heading('保存された会話を振り返り、学びの背景を確認します。','<button id="refresh">更新</button>') + `<p class="hint">更新順 · アーカイブ済みを含む会員の会話</p><section class="panel">${logTable(result.items)}</section>${pagination()}`;
      } else {
  ...
  async function loadDetail() {
    ...
    try {
      ...
      const record = result.conversation || result.document;
      $('dialogTitle').textContent = record.title || '無題';
      if (target.offset === 0) {
        $('dialogBody').replaceChildren();
        const meta = document.createElement('p');meta.className = 'hint';
        meta.textContent = result.conversation ? `${date(record.updated_at)} · ${record.archived_at?'アーカイブ済み':'保存済み'} · ID: ${record.id}` : record.source_name;
