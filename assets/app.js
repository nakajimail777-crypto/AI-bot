...
async function sendTrial(){
 ...
 if(!trialPending||trialPending.text!==text||trialPending.mode!==seikanMode||trialPending.blindSpot!==blindSpot||trialPending.emotionFocus!==emotionFocus||trialPending.encouragement!==encouragement)trialPending={text,mode:seikanMode,blindSpot,emotionFocus,encouragement,id:crypto.randomUUID()};
 status('返答を考えています…');
 const response=await fetch('/api/guest-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,requestId:trialPending.id,seikanMode,blindSpot,emotionFocus,encouragement}),signal:AbortSignal.timeout(85000)});
 const data=await response.json();
 if(version!==epoch||session)return;
 if(!response.ok){
 ...
 encouragement=false;blindSpot=false;trialRemaining=data.remaining;input.value='';trialPending=null;
 await loadTrial();
...
$('blindSpot').onclick=()=>{
 if(busy)return;
 encouragement=false;blindSpot=!blindSpot;if(blindSpot)emotionFocus=false;pending=null;trialPending=null;rememberDraft();controls();
 status(blindSpot?'次に送る内容を、違う角度から一つ照らします。':'追加の見方を取り消しました。');
};
$('emotionFocus').onclick=()=>{
 if(busy)return;
 encouragement=false;emotionFocus=!emotionFocus;if(emotionFocus)blindSpot=false;pending=null;trialPending=null;rememberDraft();controls();
 status(emotionFocus?'感情を感じきるモードに切り替えました。言葉にならない感じも、そのまま話してください。':'感情を感じきるモードをOFFにしました。');
};
let seikanMode = false;
$('seikanMode').onclick=()=>{
  if(busy)return;
  encouragement=false;seikanMode=!seikanMode;pending=null;rememberDraft();controls();
...
function rememberDraft() {
  if (!session) return;
  try { sessionStorage.setItem(storageKey(),JSON.stringify({ activeId, text:input.value, pending, cooldownUntil, seikanMode, blindSpot, emotionFocus, encouragement })); } catch {}
}
...
function renderHistory() {
  ...
  for (const chat of chats) {
    ...
    button.onclick=() => run(async()=>{ if(activeId!==chat.id){encouragement=false;blindSpot=false;emotionFocus=false;clearPdf();pending=null;input.value='';} activeId=chat.id; await loadMessages(); rememberDraft(); renderHistory(); toggleSidebar(false); });
    nav.append(button);
  }
...
async function changeSession(next) {
  ...
  session=next;
  if(oldId!==nextId){seikanMode=false;encouragement=false;blindSpot=false;emotionFocus=false;}
  ...
  try {
    ...
    if(draft){
      encouragement=draft.encouragement===true;seikanMode=draft.seikanMode===true;blindSpot=draft.blindSpot===true;emotionFocus=draft.emotionFocus===true;
...
$('composer').addEventListener('submit',event=>{
  ...
  run(async()=>{
    ...
    if(!pending||pending.text!==text||pending.conversationId!==activeId||pending.pdfId!==attachment?.id||pending.seikanMode!==seikanMode||pending.blindSpot!==blindSpot||pending.emotionFocus!==emotionFocus||pending.encouragement!==encouragement)pending={requestId:crypto.randomUUID(),conversationId:activeId,text,pdfId:attachment?.id,seikanMode,blindSpot,emotionFocus,encouragement};
    ...
    const response=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${fresh.access_token}`},body:JSON.stringify({message:text,conversationId:activeId,requestId:pending.requestId,seikanMode,blindSpot,emotionFocus,encouragement,...(attachment?{attachment:{name:attachment.name,mimeType:'application/pdf',data:attachment.data}}:{})}),signal:AbortSignal.timeout(85000)});
    ...
    encouragement=false;blindSpot=false;input.value='';pending=null;rememberDraft();status('保存しました');
...
$('newChat').onclick=()=>{if(busy)return;encouragement=false;blindSpot=false;emotionFocus=false;clearPdf();activeId=null;rows=[];pending=null;input.value='';olderMore=false;rememberDraft();renderMessages();renderHistory();status('');toggleSidebar(false);input.focus();};
$('archiveChat').onclick=()=>run(async()=>{
  ...
  encouragement=false;clearPdf();activeId=null;rows=[];pending=null;input.value='';olderMore=false;rememberDraft();renderMessages();await loadHistory();status('会話を非表示にしました。');
});
...
$('confirmDelete').onclick=()=>run(async()=>{
  ...
  try {
    ...
    if(target.version!==epoch)return;
    encouragement=false;clearPdf();activeId=null;rows=[];chats=[];pending=null;input.value='';olderMore=false;historyMore=false;historyOffset=0;
