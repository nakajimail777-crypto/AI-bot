  ...
  return async function handler(req,res) {
    ...
    try {
      ...
      for(const chunk of note.chunks) {
        const vector=await embed(`task: search result | title: ${chunk.title} | text: ${chunk.content}`,{fetcher:meter.fetch,key:geminiKey});
      ...
      await meter.flush();
      if(error.status===429) return fail(res,429,'検索用データの作成が利用上限に達しました。資料は変更していません。少し時間をおいてお試しください。');
