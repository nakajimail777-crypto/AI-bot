...
export function knowledgeQueries(text) {
  ...
  const nature = normalized.match(/龍性\s*(33|22|11|[1-9])(?!\d)/);
  const guide = normalized.match(/龍導\s*(33|22|11|[1-9])(?!\d)/);

  if (nature && guide) {
    return [`龍性${nature[1]}`, `龍導${guide[1]}`];
  }

  const numbers = dragonNumbers(text);
  return numbers
    ? [
        `龍性${numbers.nature} 龍秘術 特徴 進み方`,
        `龍導${numbers.guide} 龍秘術 特徴 進み方`
      ]
    : [text];
}
