...
export async function resolveModeInstructions({ observation = false, blindSpot = false, emotionFocus = false, encouragement = false }, { fetcher = fetch, env = process.env } = {}) {
  const modes = encouragement ? [['encouragement', true, encouragementInstruction(true)]] : [
