import { skyGazingInstruction, modeInstruction, blindSpotInstruction, emotionFocusInstruction, encouragementInstruction } from './chat-mode.js';
...
export async function resolveModeInstructions({ observation = false, blindSpot = false, emotionFocus = false, encouragement = false, returnPath = false, skyGazing = false }, { fetcher = fetch, env = process.env } = {}) {
  if (skyGazing) return skyGazingInstruction(true);
