import { modeInstruction, blindSpotInstruction, emotionFocusInstruction } from './chat-mode.js';

// Read on every generation. Never cache editable mode instructions across turns.
export async function resolveModeInstructions({ observation = false, blindSpot = false, emotionFocus = false }, { fetcher = fetch, env = process.env } = {}) {
  const modes = [
