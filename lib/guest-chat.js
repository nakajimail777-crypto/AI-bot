...
import { returnPathInstruction, returnPathMessage, returnPathReset } from './return-path.js';
...
export function createGuestHandler({env=process.env,fetcher=fetch}={}) {
 return async(req,res)=>{
  ...
  try{
   ...
   const {message,seikanMode=false,blindSpot=false,emotionFocus=false,encouragement=false,returnPath=false}=req.body||{};requestId=req.body?.requestId;
   if(typeof message!=='string'||!message.trim()||message.length>4000||!UUID.test(requestId||'')||typeof seikanMode!=='boolean'||typeof blindSpot!=='boolean'||typeof emotionFocus!=='boolean'||typeof encouragement!=='boolean'||typeof returnPath!=='boolean'||req.body?.attachment)return fail(400,'お試しでは1〜4,000文字のテキストを送信してください。');
   const savedText=returnPath?returnPathMessage(message.trim(),true):encouragement?encouragementMessage(message.trim(),true):emotionFocusMessage(blindSpotMessage(message.trim(),blindSpot),emotionFocus);
   ...
   const modeInstructions = returnPath ? returnPathInstruction(true) : await resolveModeInstructions({ observation: seikanMode, blindSpot, emotionFocus, encouragement }, { fetcher, env });
   let knowledge=[];
   try{
