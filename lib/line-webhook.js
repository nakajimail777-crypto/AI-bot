...
const NOTICE = '\n\nLINEではAIが返信します。会話内容は返答のためGoogleのAIに送信し、履歴はSupabaseに保存します。「会話をリセット」でAIが参照するLINEの履歴を消せます。';
...
export function createLineHandler({ env = process.env, fetcher = fetch, waitUntil, now = Date.now } = {}) {
  ...
  return async function handler(request) {
    ...
    const allowed = (env.LINE_ALLOWED_USER_IDS || '').split(',').map(v => v.trim()).filter(Boolean);
    const events = payload.events.filter(event => event?.source?.type === 'user'
      && /^U[0-9a-f]{32}$/.test(event.source.userId || '')
      && (!allowed.length || allowed.includes(event.source.userId))
  ...
  async function processEvent(event, receivedAt) {
    ...
    try {
      ...
      let fixed = event.type === 'follow' ? 'こんにちは。スピリットドラゴンAIです。ここでも気軽に話しかけてください。' + NOTICE
        ...
        : !text || text.length > 4000 ? 'メッセージは1〜4,000文字で送ってください。' : null;
  ...
  async function generate(text, turns, signal, meter) {
    ...
    try {
      ...
      if (embedded.ok && Array.isArray(vector) && vector.length === 768) knowledge = await db('rpc/match_knowledge', { query_embedding: vector, match_threshold: 0.58, match_count: 4 }, signal);
    ...
    const instructions = persona.trim() + context + modeInstruction(false) + blindSpotInstruction(false)
