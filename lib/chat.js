...
import { modeInstruction as getModeInstruction, blindSpotInstruction, blindSpotMessage, emotionFocusInstruction, emotionFocusMessage } from './chat-mode.js';
...
export function createHandler({ fetcher = fetch, env = process.env } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const modeInstruction=getModeInstruction(req.body?.seikanMode===true)+blindSpotInstruction(blindSpot)+emotionFocusInstruction(emotionFocus);
      let pdf;
      try { pdf = validatePdfAttachment(req.body?.attachment); }
      catch (error) { throw new HttpError(400, error.message); }
      const text = message.trim();
      const savedText = emotionFocusMessage(blindSpotMessage(pdfMessageText(text, pdf), blindSpot), emotionFocus);
