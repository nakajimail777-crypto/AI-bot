...
export function createHandler({ fetcher = fetch, env = process.env } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const modeInstruction=req.body?.seikanMode===true
      ...
      const savedText = pdfMessageText(text, pdf);
      if(savedText.length > 4000)throw new HttpError(400, 'PDFのファイル名を含めて4,000文字以内になるよう、質問を短くしてください。');
