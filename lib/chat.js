...
export function createHandler({ fetcher = fetch, env = process.env } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const savedText = encouragement ? encouragementMessage(pdfMessageText(text, pdf), true) : emotionFocusMessage(blindSpotMessage(pdfMessageText(text, pdf), blindSpot), emotionFocus);
      ...
      const modeInstruction = await resolveModeInstructions({ observation: req.body?.seikanMode === true, blindSpot, emotionFocus, encouragement }, { fetcher, env });
      let knowledge = [];
